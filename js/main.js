(function () {
    ...
    document.querySelector('.icon').addEventListener('click', (event) => {
        ...
        if (x.className == "topnav header__menu") {
            x.className = "topnav responsive";
        } else {
            x.className = "topnav header__menu";
        }
    });

    $(window).scroll(function(){
        if ($(window).scrollTop() > 100) {
            $('.header__container__inner').addClass('header-active');
        }
        else {
            $('.header__container__inner').removeClass('header-active');
        }
    });
